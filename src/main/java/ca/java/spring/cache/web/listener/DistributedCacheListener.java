 ...
 *
 */
public class DistributedCacheListener implements ServletContextListener {
	...
	logger.info("========> Distributed Cache Generation : BEGIN. ");
	...
	if (null != ctx) {
	    ...
	    logger.info("========> Distributed Cache Generation : YMM was added in cache." +yearMakeModel.getEnglish().keySet());
	    YearMakeModel yearMakeModeInCache = (YearMakeModel)ehcache.get(ALL_KEY).getObjectValue();
	    logger.info("========> Distributed Cache Generation : YMM from cache." +yearMakeModeInCache.getEnglish().keySet());
	} else {
